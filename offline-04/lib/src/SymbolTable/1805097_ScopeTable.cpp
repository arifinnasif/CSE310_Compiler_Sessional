...
bool ScopeTable::insert(string arg_name, string arg_type, int * pos1, int * pos2) {
    ...
    if(buckets[h] == NULL) {
        ...
        buckets[h]->table_id = this->getAbsoluteID();
        if(pos1 != NULL) *pos1 = h;
    ...
    while(temp->getName().compare(arg_name) != 0) {
        ...
        if(temp->getNext() == NULL) {
            SymbolInfo* to_insert = new SymbolInfo(arg_name, arg_type, &buckets[h], temp, NULL);
            temp->setNext(to_insert);
            to_insert->table_id = this->getAbsoluteID();
            if(pos1 != NULL) *pos1 = h;
            if(pos2 != NULL) *pos2 = cnt;
            //os<<"Inserted in ScopeTable# "<<getAbsoluteID()<<" at position "<<h<<", "<<cnt<<""<<endl;
            return true;
...
bool ScopeTable::insert(SymbolInfo * argSymbolInfo, int * pos1, int * pos2) {
    ...
    if(buckets[h] == NULL) {
        ...
        if(pos2 != NULL) *pos2 = cnt;
        //os<<"Inserted in ScopeTable# "<<getAbsoluteID()<<" at position "<<h<<", "<<cnt<<""<<endl;
        argSymbolInfo->table_id = this->getAbsoluteID();
        return true;
    }
    SymbolInfo * temp = buckets[h];
